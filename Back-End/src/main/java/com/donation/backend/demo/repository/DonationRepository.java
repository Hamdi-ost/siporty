...
public interface DonationRepository extends JpaRepository<Donation, Long> {
    ...
    @Query(value = "SELECT * FROM donation WHERE SUBSTR(donation.date, 4, 2) = ?1 AND SUBSTR(donation.date, 7, 4) = ?2 ORDER BY donation.montant DESC LIMIT 10", nativeQuery = true)
    List<Donation> getTopDonorsPerMonthAll(String m, String y);
    ...
    @Query(value = "SELECT * FROM donation WHERE STR_TO_DATE(date,'%d/%m/%Y') BETWEEN STR_TO_DATE(?1,'%d/%m/%Y') AND STR_TO_DATE(?2,'%d/%m/%Y') ORDER BY donation.montant DESC LIMIT 10", nativeQuery = true)
    List<Donation> getTopDonorsPerWeekAll(String m, String y);
